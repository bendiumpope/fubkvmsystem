...
import com.fubk_visitor_system.repository.DatabaseFactory.dbQuery
...
class VisitRepository : Repository {
    ...
    override suspend fun addvisitingschedule(
        ...
        datebookedValue: String
    ) = dbQuery {
            val insertStatement = VisitingSchedules.insert {
                it[user] = userId
                it[office] = officeValue
                it[date] = dateValue
                it[timeduration] = durationValue
                it[datebooked] = datebookedValue
        }
    val result = insertStatement.resultedValues?.get(0)
    if(result != null){
        toVisitingSchedule(result)
    }else{
        null
    }
    }
    ...
    private fun toVisitingSchedule(row: ResultRow): VisitingSchedule =
        VisitingSchedule(
            ...
            datebooked = row[VisitingSchedules.datebooked]
        )
}